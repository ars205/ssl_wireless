...
from models import ChannelTransformationModule as channel_transforms
importlib.reload(channel_transforms)
print(importlib.reload(channel_transforms))
            ...
            for ff in range(realizations):
                ...
                for i in range(3):
                    ...
                    for tensor_channel, spot in data_loader:
                        ...
                        if i==2:
                            h_input = global_transfo2_test2(h_input)
                        else:
                            h_input = global_transfo2_test(h_input)
                        h_input = torch.squeeze(h_input, 0)
                        h_input_feat = get_embeddings(model.to('cuda'), h_input.to('cuda'), which_o='MEAN').T
                ...
                optimizer = optim.SGD(net.parameters(), lr=args.learning_rate_eval, momentum=0.1, weight_decay = 0)
                print(net)
                ...
                for epoch in range(epochs):
                    ...
                    with torch.no_grad():
                        ...
                        for data, label in val_loader2:
                            ...
                            epoch_val_loss += val_loss / len(val_loader2)
